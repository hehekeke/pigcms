...
<script type="text/javascript">
	...
	var site_key = "<?php echo ($pc_config["site_tpl"]["key"]); ?>";
    $(function(){
		if(site_key == ''){
		...
		$('#change_tpl').click(function(){
			if(site_url == ''){
				...
				tpl_iframe = art.dialog.open("<?php echo U('Web/location_tpl');?>&key=<?php echo ($pc_config["site_tpl"]["key"]); ?>&site_url="+site_url,{lock:true,title:'选择模板',width:art_width,height:'100%',fixed:true,close:function(){$.getJSON("http://"+site_url+"/index.php?g=Web&m=Web_index&a=unset_tpl_session&callback=?");},background: '#000',opacity: 0.45});
			}
		});
    });
	...
	function getTpl(){
		...
		setTimeout(function(){
			if(site_key != ''){
				$.getJSON("<?php echo C('web_default_set.static_domain');?>/info.php?key="+site_key+"&jsonp=?",function(result){
					if(result['error'] == 0){
						var  tpl_info = '<div style="float:left;">模板名称：&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">'+result['name']+'</font><br/><br/><br/>';
							tpl_info += '模板预览：&nbsp;&nbsp;&nbsp;&nbsp;<img src="'+result['preview']+'" style="vertical-align:middle;border:1px solid #ccc;"/></div><div style="float:left;margin-left:280px;padding-top:60px;"><a href="http://<?php echo ($first_pc_site); ?>" target="_blank"><img src="<?php echo RES;?>/images/see_web.png" style="width:120px;"/></a></div>';
