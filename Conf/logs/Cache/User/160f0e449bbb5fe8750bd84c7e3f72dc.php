...
$(function(){
	$('.del_confirm').click(function(){
		if(!confirm('若您选择的导航包含子级导航，将会同时删除子级导航，是否确认删除？')){
			return false;
		}
	});
});
...
<div class="cLineB">
  <h4>导航菜单管理  <span class="FAQ"></span></h4>
 </div>
         <div class="cLineC" style="line-height: 32px;">
 <a href="<?php echo U('Web/nav',array('act'=>'nav_add'));?>" class="btnGrayS vm bigbtn">添加导航</a>　 
</div>
...
<th>关键词 <a target="999" class="tooltips" ><img src="<?php echo RES;?>/images/price_help.png" align="absmiddle" /><span>
<p>可填写关键词，用来获取子导航列表、单个导航的内容！可以为空</p>
</span></a></th>
<th>链接地址</th>
                        <th class="norightborder">操作</th>
</tr>
</thead>
<?php if(is_array($pc_nav)): $i = 0; $__LIST__ = $pc_nav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
	<td><b><?php echo ($vo["name"]); ?></b></td>
	<td><?php echo ($vo["key"]); ?></td>
	<td><?php echo ($vo["url"]); ?></td>
	<td class="norightborder" width="200">
		<a href="<?php echo U('Web/nav',array('act'=>'nav_edit','id'=>$vo['id']));?>">编辑</a>&nbsp;&nbsp;&nbsp;
		<a href="<?php echo U('Web/nav',array('act'=>'nav_del','id'=>$vo['id']));?>" class="del_confirm">删除</a>
		</nobr>
	</td>
</tr>
	<?php if(is_array($vo['nav_list'])): $i = 0; $__LIST__ = $vo['nav_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><tr>
			<td> --------- <?php echo ($voo["name"]); ?></td>
			<td><?php echo ($voo["key"]); ?></td>
			<td><?php echo ($voo["url"]); ?></td>
			<td class="norightborder" width="200">
				<a href="<?php echo U('Web/nav',array('act'=>'nav_edit','id'=>$voo['id']));?>">编辑</a>&nbsp;&nbsp;&nbsp;
				<a href="<?php echo U('Web/nav',array('act'=>'nav_del','id'=>$voo['id']));?>" class="del_confirm">删除</a>
				</nobr>
			</td>
