              ...
              <TBODY>
				 <?php if(is_array($members)): $i = 0; $__LIST__ = $members;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><TR>
                  ...
                  <TD>
                    ...
                    <TD>
                        <?php echo ($list["keshi"]); ?>
                        <form method="post"  action="/index.php?g=User&m=Member_card&a=keshiExpense&token=<?php echo ($_GET['token']); ?>&id=<?php echo ($thisCard["id"]); ?>" >
                            <input type="text" name="ks" value="0" class="px" style="width:40px;">
                            <input type="hidden" name="wecha_id" value="<?php echo ($list["wecha_id"]); ?>">
                            <button type="submit" style="cursor:pointer" class="btnGrayS vm">设置</button>
                        </form>
                    </TD>
                  <TD class="norightborder">				  
				       ...
				       |
                      <a href="<?php echo U('Member_card/keshi',array('token'=>$token,'cardid'=>$thisCard['id'],'number'=>$list['number']));?>" ><strong>课时记录</strong></a>
                      |
