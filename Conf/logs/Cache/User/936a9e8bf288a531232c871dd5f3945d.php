...
<div class="cLine">
    <div class="msgWrap form">
        <ul id="tags" style="width:100%">
            <li class="selectTag">
                <a href="<?php echo U('User/Weneed/index',array('token'=>$token));?>">需求大厅</a>
            </li>

            <li >
                <a href="<?php echo U('User/Weneed/jd',array('token'=>$token));?>">接单大厅</a>
            </li>

            <li>
                <a href="<?php echo U('User/Weneed/user',array('token'=>$token));?>">用户管理</a>
            </li>
...
<TBODY>
<TR></TR>
<?php if(is_array($needs)): $i = 0; $__LIST__ = $needs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><TR>
<TD><?php echo ($item["id"]); ?></TD>
<TD><?php echo ($item["title"]); ?></TD>
<TD><?php echo ($types[$item['need_type']]['name']); ?></TD>
<TD><?php echo ($types[$item['need_type']]['name']); ?></TD>
    <TD><?php echo ($item["money"]); ?></TD>
    <TD><?php echo (date('Y-m-d H:i',$item["create_time"])); ?></TD>
<TD>

<a  onclick="showdetail('#detail<?php echo ($item["id"]); ?>')">查看详情</a>
|
<a href="javascript:drop_confirm('您确定要删除吗?', '/index.php?g=User&m=Weneed&a=del_need&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>');">删除</a></TD>
</TR>
    <tr id="detail<?php echo ($item["id"]); ?>" style="display: none" onclick="hidedetail('#detail<?php echo ($item["id"]); ?>')">
        <TD colspan="6" style="background-color: #F3ECB9;color: #930" >
            需求详情：<?php echo (($item["detail"])?($item["detail"]):'N/A'); ?><br>
            联系方式：<?php echo (($item["contact"])?($item["contact"]):'N/A'); ?><br>
            具体地址：<?php echo (($item["address"])?($item["address"]):'N/A'); ?><br>
            接单用户：<?php echo (($item["jid"])?($item["jid"]):'N/A'); ?><br>
