...
<link rel="stylesheet" type="text/css" href="<?php echo ($staticPath); ?>/tpl/static/yundabao/css/cymain.css" />
<div class="content">
	<div class="cLineB">
	...
	<div class="tab">
		<ul>
			<li class="<?php if($_GET['type'] == "" || $_GET['type'] == "0"){?>current <?php }?>tabli" id="tab1"><a href="<?php echo U('Yundabao/applist',array('token'=>$token,'type'=>'0'));?>">所有应用</a></li>
			<li class="<?php if($_GET['type'] == "1"){?>current <?php }?>tabli" id="tab2"><a href="<?php echo U('Yundabao/applist',array('token'=>$token,'type'=>'1'));?>">未到期应用</a></li>
			<li class="<?php if($_GET['type'] == "2"){?>current <?php }?>tabli" id="tab3"><a href="<?php echo U('Yundabao/applist',array('token'=>$token,'type'=>'2'));?>">已到期应用</a></li>
		</ul>
	</div>
		...
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<tbody>
				<?php if(is_array($AppList)): $i = 0; $__LIST__ = $AppList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><img src="<?php echo ($vo["IconUrl"]); ?>" width="100%" /></td>
						<td><?php echo ($vo["AppName"]); ?></td>
						<td><?php echo ($vo["PackName"]); ?></td>
						<td><?php echo ($vo["AppVer"]); ?></td>
						<td><?php echo ($vo["UseRange"]); ?></td>
						<td>
							<a href="<?php echo U('Yundabao/download',array('token'=>$token,'AppId'=>$vo['AppId'],'filetype'=>'apk'));?>" target="_blank">点击下载安卓版</a>
							<br/>
							<a href="<?php echo U('Yundabao/download',array('token'=>$token,'AppId'=>$vo['AppId'],'filetype'=>'ipa'));?>" target="_blank">点击下载苹果版</a>
						</td>
						<td class="norightborder" >
							<a href="<?php echo U('Yundabao/update',array('token'=>$token,'AppId'=>$vo['AppId']));?>">修改</a>
							<a href="<?php echo U('Yundabao/RebuildApp',array('token'=>$token,'AppId'=>$vo['AppId']));?>">重新生成</a>
							<a href="<?php echo U('Yundabao/data',array('token'=>$token,'AppId'=>$vo['AppId']));?>">数据</a>
							<!-- <a href="javascript:drop_confirm('您确定要删除【分类1】吗?', '/index.php?g=User&m=LivingCircle&a=operate&token=hjbrhl1419386992&typeid=1&type=typedel')">删除</a> -->
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["AppName"]); ?>】吗?','<?php echo U('Yundabao/DeleteApp',array('token'=>$token,'AppId'=>$vo['AppId']));?>')">删除</a>
						</td>
