...
<div class="content">
	...
	<div class="tab">
		<ul>
			...
			<li class="current tabli" id="tab2"><a href="<?php echo U('LivingCircle/sellcircle',array('token'=>$token));?>">商圈管理</a></li>
	...
	<div class="cLine">
		<div class="pageNavigator left">
			<a href="<?php echo U('LivingCircle/sellcircleadd',array('token'=>$token));?>" title="添加商圈" class="btnGrayS vm bigbtn">添加商圈</a>
		</div>
		<div class="pageNavigator right">
			<input type="text" id="name" class="px name" placeholder="输入商圈名称"  name="name" value="<?php echo ($_GET['name']); ?>" style="margin-top:10px">
	...
	<div class="msgWrap">
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<tbody>
				<?php if(is_array($sellcircle_list)): $i = 0; $__LIST__ = $sellcircle_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo ($vo["num"]); ?></td>
					<td><?php echo ($vo["name"]); ?></td>
					<?php if($vo["display"] == 1): ?><td><a href="javascript:display(<?php echo ($vo["pigcms_id"]); ?>)"><img id="display<?php echo ($vo["pigcms_id"]); ?>" src="<?php echo ($staticPath); ?>/tpl/static/livingcircle/images/start.png" width="100%"/></a></td>
					<?php else: ?>
					<td><a href="javascript:display(<?php echo ($vo["pigcms_id"]); ?>)"><img id="display<?php echo ($vo["pigcms_id"]); ?>" src="<?php echo ($staticPath); ?>/tpl/static/livingcircle/images/stop.png" width="100%"/></a></td><?php endif; ?>
					<td class="norightborder" style="border-right:1px solid #eee">
						<a href="<?php echo U('LivingCircle/zisellcircle',array('token'=>$token,'sellcircleid'=>$vo['pigcms_id']));?>">子商圈</a>
						<a href="<?php echo U('LivingCircle/sellcircleupdate',array('token'=>$token,'sellcircleid'=>$vo['pigcms_id']));?>">修改</a>
						<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["name"]); ?>】吗?', '<?php echo U('LivingCircle/operate',array('token'=>$token,'sellcircleid'=>$vo['pigcms_id'],'type'=>'sellcircledel'));?>')">删除</a>
					</td>
...
<script>
	...
	function seach(){
		var name = $("#name").val();
		if(name != ""){
			window.location.href="<?php echo U('LivingCircle/sellcircle',array('token'=>$token));?>&name="+name;
		}else{
			window.location.href="<?php echo U('LivingCircle/sellcircle',array('token'=>$token));?>";
		}
	}
</script>
</div>
