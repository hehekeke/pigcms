...
<a href="<?php echo U('Voteimg/index',array('token'=>$token));?>" class="right btnGrayS vm" style="margin-top:1px">返回投票列表</a>
	...
	<?php if($list != ''): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tbody><tr>
	   <td><?php echo ($vo["baby_id"]); ?></td>
		  <td><?php echo (mb_substr($vo["vote_title"],0,8,'utf-8')); ?></td>
		  <td><img src="<?php echo ($vo["vote_img"]); ?>" width="50px" height="50px"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="art.dialog.open('<?php echo U('Voteimg/apply_phone_list',array('id'=>$vo['id']));?>',{lock:false,title:'报名图片预览',width:800,height:600,yesText:'关闭',background: '#000',opacity: 0.87});" href="###">预览</a></td>
		  <td><?php echo ($vo["jump_url"]); ?></td>
		  <td><?php echo ($vo["vote_count"]); ?></td>
		   <td><?php echo (($vo["contact"])?($vo["contact"]):"暂无"); ?></td>
		  <td class="norightborder">
		  <a href="<?php echo U('Voteimg/edit_item',array('id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">编辑选项</a>
		  <a href="<?php echo U('Voteimg/del_item',array('id'=>$vo['id'],'vote_id'=>$vo['vote_id'],'token'=>$vo['token']));?>" onclick="return(confirm('确定要删除吗？'))" class="btnGreen">删除</a> </td>
		</tr>
